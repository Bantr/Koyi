...
export class NotificationService {
    ...
    constructor(
        ...
        this.bot.on('ready', () => {
            ...
            this.logger.log(`Invite link: https://discordapp.com/oauth2/authorize?client_id=${configService.get('BANTR_DISCORD_CLIENTID')}&scope=bot&permissions=3072`);
        });
    }
    ...
    async sendNotification(options: INotificationOptions) {
        ...
        this.logger.log(`Sending a notification to ${users.length} users about ${player.steamId}`);
        for (const user of users) {

            const settings = user.settings;

            const delBans = this.filterDataBasedOnSettings(deletedBans.filter(_ => !_.preExisting), settings);
            const addBans = this.filterDataBasedOnSettings(newBans.filter(_ => !_.preExisting), settings);
            ...
            if (addBans.length || delBans.length) {
                ...
                if (settings.notificationDiscordEnabled) {
                    try {
                        await this.sendDiscordDm(user, { deletedBans: delBans, newBans: addBans, player });
                    } catch (error) {
                        this.logger.error(`Error sending Discord DM to user ${user.id} - ${error}`);
                    }
                }
            }
        }
    ...
    public filterDataBasedOnSettings(bans: Ban[], settings: UserSettings) {
        ...
        if (settings.notificationEconomyEnabled) {
            filteredBans = filteredBans.concat(this.filterOnBanType(IBanType.Economy, bans));
        }

        if (settings.notificationVACEnabled) {
            filteredBans = filteredBans.concat(this.filterOnBanType(IBanType.VAC, bans));
        }

        if (settings.notificationGameEnabled) {
            filteredBans = filteredBans.concat(this.filterOnBanType(IBanType.Game, bans));
        }

        if (settings.notificationCommunityEnabled) {
            filteredBans = filteredBans.concat(this.filterOnBanType(IBanType.Community, bans));
        }

        if (settings.notificationFaceitEnabled) {
            filteredBans = filteredBans.concat(this.filterOnBanType(IBanType.Faceit, bans));
        }
        return filteredBans;
    }
    ...
    public async sendToGlobalChannel(options: INotificationOptions) {
        const channelId = this.configService.get('BANTR_GLOBAL_NOTIFICATION_DISCORD');
    ...
    public createDiscordEmbed(options: INotificationOptionsEmbed) {
        ...
        embed.addField('🕒 Tracking since', `${player.createdAt.toLocaleDateString()}`, true);
    ...
    public async testDiscordNotification(user: User) {
        ...
        try {
            const embed = await this.createDiscordEmbed({ deletedBans, newBans, player });
